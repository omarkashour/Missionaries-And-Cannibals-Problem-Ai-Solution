...
public class DFSscreen extends BorderPane {
	...
	Button runBtn = new Button("Run DFS");
	...
	public DFSscreen(Stage primaryStage, Scene scene) {
		...
		runBtn.setOnAction(e -> {
			...
			runTimeL.setText("Executed DFS in " + result + " ms");
	...
	public static boolean dfs(State initialState) {
		...
		while (!stack.isEmpty()) {
			State s = stack.pop();
			if (isGoal(s)) {
				System.out.println("We reached the solution using DFS!, " + s + " in " + steps + " steps!");
				solutionSteps.add(s);
				return true;
			}
